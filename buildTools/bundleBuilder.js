...
function build(bundlesDir, bundleName, node_modules, options) {
	...
	var testableHelper = concat(path.join(bundlePath, helperName), {
 	...
 	var testableRenderer = concat(path.join(bundlePath, rendererName), {
  ...
  allNodes.push(jshintTests);
	...
	var controller = deploy(pickFiles(bundlePath, { include: [ controllerName ] }), 
		Object.assign({}, deploymentDefaults, { 
		file: controllerName,
		defType: 'CONTROLLER',
		format: 'JS' 
